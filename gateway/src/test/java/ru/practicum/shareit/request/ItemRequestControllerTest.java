...
class ItemRequestControllerTest {
    ...
    @BeforeEach
    public void setUp() {
        addItemRequestDto = new ItemRequestDtoRequest("description");
        itemResponseDto = new ItemRequestDtoResponse();
    ...
    public void addNewItemRequest_ValidRequest_ShouldReturnRequest() {
        ...
        mvc.perform(post("/requests")
                ...
                .andExpect(content().string(objectMapper.writeValueAsString(itemResponseDto)))
                .andExpect(jsonPath("$.id", is(itemResponseDto.getId())))
                .andExpect(jsonPath("$.description", is(itemResponseDto.getDescription())))
                .andExpect(jsonPath("$.created", is(itemResponseDto.getCreated())))
                .andExpect(jsonPath("$.items", is(itemResponseDto.getItems())));
    ...
    public void getAllItemRequestsFromUser_Valid_ShouldReturnRequest() {
        ...
        mvc.perform(get("/requests")
                ...
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().string(objectMapper.writeValueAsString(List.of(itemResponseDto))))
                .andExpect(jsonPath("$.length()", is(1)))
                .andExpect(jsonPath("$.[0].id", is(itemResponseDto.getId())))
                .andExpect(jsonPath("$.[0].description", is(itemResponseDto.getDescription())))
                .andExpect(jsonPath("$.[0].created", is(itemResponseDto.getCreated())))
                .andExpect(jsonPath("$.[0].items", is(itemResponseDto.getItems())));
